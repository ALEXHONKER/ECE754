...
%% W is weight, MU is u(the center vector),
%% K-means
...
%% K-means
CENTS = F( ceil(rand(K,1)*size(F,1)) ,:);             % Cluster Centers, randomly select k points as the center
...
%% MU SIGMA K
MU    = zeros(size(F,2),K);                           % MU: the center of each cluster
...
for i = 1:K
    ...
    SIGMA(i)=su/size(cul,1);
    %SIGMA(:,:,i) = diag(std(F(DAL(:,K+1) == i,:))); 
